...
function displayName(u: ClientUser): string {
...
function UserCard({ user, onRefresh }: { user: ClientUser; onRefresh: () => void }) {
...
export default function ClientsPage() {
  ...
  const load = useCallback(async (p = page) => {
    ...
    setApiDown(false);
    try {
      const res = await kkGet<PageData>(`v1/admin/users/by-role?role=client&page=${p}`);
      setData(res);
    } catch (e: unknown) {
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-kob-text tracking-tight flex items-center gap-2">
            <Users className="h-5 w-5 text-kob-gold" />
            Client Users
          </h1>
          <p className="text-xs text-kob-muted mt-0.5">
            Personal KobKlein accounts · {data?.total ?? 0} total
          </p>
        </div>
        <button
          type="button"
      ...
      {data && (
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-2xl border border-kob-gold/20 bg-[#080E20] p-4">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-16 gap-2">
          ...
          <span className="text-xs text-kob-muted">Loading clients…</span>
        </div>
      ) : users.length === 0 ? (
        <div className="rounded-2xl border border-white/8 bg-[#080E20] flex flex-col items-center justify-center py-14 gap-3">
          ...
          <p className="text-sm font-semibold text-kob-text">No client users yet</p>
        </div>
      ) : (
        <div className="space-y-2">
          {users.map((u) => (
            <UserCard key={u.id} user={u} onRefresh={() => load()} />
          ))}
        </div>
      )}
